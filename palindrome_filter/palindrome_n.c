...
void writer(FILE *write)
{
    ...
    { // Finchè vi sono righe da leggere

        printf("%s\n", line);
        if (!strncmp(line, "<terminated>", BUF_SIZE))
            break;
    }
    exit(0);
}
